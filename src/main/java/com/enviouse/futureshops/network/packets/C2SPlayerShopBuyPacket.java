...
public record C2SPlayerShopBuyPacket(BlockPos shopPos, int quantity) {
    ...
    public static C2SPlayerShopBuyPacket decode(FriendlyByteBuf buffer) {
        return new C2SPlayerShopBuyPacket(buffer.readBlockPos(), buffer.readVarInt());
    }
    ...
    public static void handle(C2SPlayerShopBuyPacket packet, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ...
            if (player != null) {
                PlayerShopBlockService.buy(player, packet.shopPos(), packet.quantity());
            }
        });
