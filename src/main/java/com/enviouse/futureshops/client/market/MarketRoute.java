...
public record MarketRoute(
    ...
    private static final int MAX_SELECTION_LENGTH = 192;
    ...
    public MarketRoute {
        ...
        selectionId = bounded(selectionId, MAX_SELECTION_LENGTH, true, "Market selection identifier");
    ...
    public MarketRoute toDetail(String selection, UUID nonce) {
        if (isDetailView(module, viewId)) {
            throw new IllegalStateException("Market route is already a detail route.");
        }
        if (selection == null || selection.isBlank()) {
            throw new IllegalArgumentException(
                "Market detail selection is required.");
        }
        return new MarketRoute(module, detailView(module), categoryId,
            query, filterId, sortId, page, scrollOffset, selection, nonce);
    }
    ...
    public boolean isDetail() {
        return isDetailView(module, viewId);
    }

    public static String detailView(MarketModule module) {
        return switch (Objects.requireNonNull(module, "module")) {
            case SHOP -> "item_detail";
            case BAZAAR -> "product_detail";
            case AUCTION_HOUSE -> "listing_detail";
        };
    }

    public static boolean isDetailView(
        MarketModule module,
        String viewId
    ) {
        return detailView(module).equals(
