...
public class PlayerShopBarterScreen extends Screen implements ShopScreenMarker {
    ...
    protected void init() {
        ...
        // ═══ Bottom row: Qty controls left | receive summary center | Confirm right ═══
        // The ± / Max / Confirm / Back buttons are flat Nocturne primitives drawn immediate-mode
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Back button top-left — always available, even when no listing is selected.
        ShopUiUtil.button(graphics, this.font, clickZones, mouseX, mouseY,
                guiLeft + 6, guiTop + 6, 44, 14,
                Component.translatable("gui.futureshops.player_shop_barter.back"),
                ShopUiUtil.ButtonStyle.SECONDARY, true, this::onClose);

        if (listing == null) {
            graphics.drawCenteredString(this.font, Component.translatable("gui.futureshops.player_shop_barter.no_listing"),
        ...
        String totalText = Component.translatable("gui.futureshops.player_shop_barter.receive_summary",
                totalOutput,
                this.font.plainSubstrByWidth(ShopUiUtil.getItemDisplayNameWithNbtAndQty(listing.itemId(), listing.nbtJson(), listing.baseQuantity()), 100)).getString();
        ...
        if (plusHov) hoveredTooltip = Component.translatable("gui.futureshops.player_shop_barter.tooltip.shift_max");
        ShopUiUtil.button(graphics, this.font, clickZones, mouseX, mouseY,
                qtyX + 70, bottomY, 28, 16, Component.translatable("gui.futureshops.player_shop_barter.max"),
                ShopUiUtil.ButtonStyle.SECONDARY, true, () -> setQuantity(resolveMaxQuantity()));

        // Confirm — disabled until stock covers the quantity and the player owns enough barter items.
        ...
        int owned = (barterId != null && !barterId.isBlank())
                ...
                : 0;
        boolean canConfirm = listing.stock() >= quantity && owned >= needed;
        ShopUiUtil.button(graphics, this.font, clickZones, mouseX, mouseY,
                guiLeft + guiW - 90, bottomY, 80, 16, Component.translatable("gui.futureshops.barter.confirm"),
                ShopUiUtil.ButtonStyle.PRIMARY, canConfirm, this::showBarterConfirmation);
    ...
    private void renderReceivePanel(GuiGraphics graphics, int x, int y, int w, int h, PlayerShopListingData listing, int mouseX, int mouseY) {
        ...
        graphics.drawString(this.font, Component.translatable("gui.futureshops.player_shop_barter.you_receive"), x + 8, y + 6, ShopColors.STATUS_SUCCESS, false);

        // NBT-aware large preview — center in available space above text stack.
        // Always apply the wire NBT for display: nbtAware only governs transaction
        // matching, and BEWLR items (TacZ guns etc.) need the tag to render.
        ...
        String stockText = listing.stock() > 999
                ? Component.translatable("gui.futureshops.player_shop_barter.stock_unlimited").getString()
                : Component.translatable("gui.futureshops.player_shop_barter.stock_count", listing.stock()).getString();
    ...
    private void renderGivePanel(GuiGraphics graphics, int x, int y, int w, int h, PlayerShopListingData listing, int mouseX, int mouseY) {
        ...
        graphics.drawString(this.font, Component.translatable("gui.futureshops.player_shop_barter.you_give"), x + 8, y + 6, ShopColors.TEXT_BARTER_SOFT, false);

        String barterId = listing.barterItemId();
        if (barterId == null || barterId.isBlank()) {
            graphics.drawCenteredString(this.font, Component.translatable("gui.futureshops.player_shop_barter.not_configured"),
        ...
        // Item preview in available space — NBT-aware so enchanted / tagged barter
        // items render with their correct visual (enchantment glint, gun models).
        ...
        String ownedStr = Component.translatable("gui.futureshops.player_shop_barter.have", owned).getString();
    ...
    private void showBarterConfirmation() {
        ...
        summary.add(ConfirmationModal.SummaryLine.item(
                ...
                Component.translatable("gui.futureshops.player_shop_barter.confirm.give", giveName, needed).getString(),
                listing.barterNbtJson()));
        summary.add(ConfirmationModal.SummaryLine.item(
                listing.itemId(),
                Component.translatable("gui.futureshops.player_shop_barter.confirm.receive", receiveName, quantity).getString(),
                listing.nbtJson()));
        ...
        if (compound) {
            ...
            totalText = Component.translatable("gui.futureshops.player_shop_block.confirm.total_compound", moneyStr, needed, giveName).getString();
        } else {
