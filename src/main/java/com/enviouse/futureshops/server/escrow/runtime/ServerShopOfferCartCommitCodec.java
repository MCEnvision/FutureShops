...
public final class ServerShopOfferCartCommitCodec {
    public static final int MAX_ENCODED_BYTES = 40 * 1_048_576;
    private static final int MAGIC = 0x46534f43;

    private ServerShopOfferCartCommitCodec() {
    }

    public static byte[] encode(ServerShopOfferCartCommit commit) {
        try {
            ...
            for (ServerShopOfferCartCommit.Line line : commit.lines()) {
                output.writeUTF(line.listingId());
                output.writeUTF(line.optionId());
                output.writeInt(line.quantity());
                output.writeLong(line.offerRevision());
                output.writeUTF(line.listingFingerprint());
                output.writeBoolean(line.savings().isPresent());
                if (line.savings().isPresent()) {
                    writeSavings(output, line.savings().orElseThrow());
                }
            }
            ...
            throw new IllegalStateException(
                    "Unable to encode server shop offer cart commit",
                    exception);
        }
    }

    public static ServerShopOfferCartCommit decode(byte[] encoded) {
        requireSize(encoded);
        try {
            ...
            if (input.readInt() != MAGIC) {
                throw new IllegalArgumentException(
                        "Server shop offer cart magic is invalid");
            }
            ...
            for (int index = 0; index < lineCount; index++) {
                ...
                Optional<ServerShopBundleSavings.Snapshot> savings =
                        input.readBoolean()
                                ...
                                : Optional.empty();
                lines.add(new ServerShopOfferCartCommit.Line(
                        listingId, optionId, quantity, revision,
                        fingerprint, savings));
            }
            ...
            if (bytes.available() != 0
                    || !(reserve instanceof
                    StockMutationCommand.ReserveBatch reserveBatch)
                    || !(commit instanceof
                    StockMutationCommand.ResolveBatch commitBatch)) {
                throw new IllegalArgumentException(
                        "Server shop offer cart commit shape is invalid");
            }
            ServerShopOfferCartCommit result =
                    new ServerShopOfferCartCommit(
                            schema, requestId, playerId, shopId, lines,
                            paymentSource, quotedAt, claimsPending, value,
                            reserveBatch, commitBatch, fingerprint);
            if (!Arrays.equals(encoded, encode(result))) {
                throw new IllegalArgumentException(
                        "Server shop offer cart commit is not canonical");
            }
            return result;
            ...
            throw new IllegalArgumentException(
                    "Server shop offer cart commit is malformed",
                    exception);
        }
    }
    ...
    private static ServerShopBundleSavings.Snapshot readSavings(
        ...
        if (count <= 0 || count > 36) {
            throw new IllegalArgumentException(
                    "Server shop offer cart savings count is invalid");
        }
    ...
    private static byte[] readBytes(DataInputStream input)
        ...
        if (length <= 0 || length > MAX_ENCODED_BYTES) {
            throw new IllegalArgumentException(
                    "Server shop offer cart component size is invalid");
        }
        byte[] value = input.readNBytes(length);
        if (value.length != length) {
            throw new IllegalArgumentException(
                    "Server shop offer cart component is truncated");
        }
        return value;
    }
    ...
    private static <T> T enumValue(T[] values, int ordinal) {
        if (ordinal < 0 || ordinal >= values.length) {
            throw new IllegalArgumentException(
                    "Server shop offer cart enum is invalid");
        }
    ...
    private static void requireSize(byte[] encoded) {
        if (encoded == null || encoded.length == 0
            ...
            throw new IllegalArgumentException(
                    "Server shop offer cart commit size is invalid");
        }
    }
}
