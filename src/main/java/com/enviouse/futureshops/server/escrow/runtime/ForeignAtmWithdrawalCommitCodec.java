...
public final class ForeignAtmWithdrawalCommitCodec {
    public static final int CURRENT_SCHEMA = 1;
    public static final int MAX_ENCODED_BYTES = 8_388_608;

    private static final int MAGIC = 0x46534641;
    private static final int MAX_LEDGER_BYTES = 16_384;
    private static final int MAX_CLAIM_BYTES =
            ForeignCashClaimPayloadCodec.MAX_ENCODED_BYTES + 4096;

    private ForeignAtmWithdrawalCommitCodec() {
    }

    public static byte[] encode(ForeignAtmWithdrawalCommit commit) {
        ...
        try {
            ...
            writeComponent(output,
                    ...
                    MAX_ENCODED_BYTES,
                    "Foreign ATM withdrawal transaction");
            writeComponent(output,
                    LedgerJournalCodec.encode(commit.ledgerTransaction()),
                    MAX_LEDGER_BYTES,
                    "Foreign ATM withdrawal ledger");
            ...
            for (EscrowClaim claim : commit.cashClaims()) {
                writeComponent(output, ClaimJournalCodec.encodeClaim(claim),
                        MAX_CLAIM_BYTES,
                        "Foreign ATM withdrawal cash claim");
            }
            output.flush();
            ...
            if (encoded.length == 0 || encoded.length > MAX_ENCODED_BYTES
                ...
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal commit exceeds its binary limit");
            }
            return encoded;
        } catch (IOException exception) {
            throw new IllegalStateException(
                    "Unable to encode foreign ATM withdrawal commit", exception);
        }
    }

    public static ForeignAtmWithdrawalCommit decode(byte[] encoded) {
        if (encoded == null || encoded.length == 0
                || encoded.length > MAX_ENCODED_BYTES) {
            throw new IllegalArgumentException(
                    "Foreign ATM withdrawal commit size is invalid");
        }
        ByteArrayInputStream bytes = new ByteArrayInputStream(encoded);
        try (DataInputStream input = new DataInputStream(bytes)) {
            if (input.readInt() != MAGIC) {
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal commit magic is invalid");
            }
            int schema = input.readInt();
            if (schema > CURRENT_SCHEMA) {
                throw new IllegalStateException(
                        "Foreign ATM withdrawal commit schema is newer than this build");
            }
            if (schema != CURRENT_SCHEMA) {
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal commit schema is unsupported");
            }
            ...
            if (claimCount <= 0
                ...
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal cash claim count is invalid");
            }
            ...
            if (bytes.available() != 0) {
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal commit has trailing data");
            }
            ForeignAtmWithdrawalCommit commit =
                    new ForeignAtmWithdrawalCommit(requestId, playerId,
                            transaction, ledger, claims);
            if (!claims.equals(commit.cashClaims())) {
                throw new IllegalArgumentException(
                        "Foreign ATM withdrawal commit ordering is not canonical");
            }
            return commit;
            ...
            throw new IllegalArgumentException(
                    "Foreign ATM withdrawal commit is truncated", exception);
        } catch (IOException | RuntimeException exception) {
            throw new IllegalArgumentException(
                    "Foreign ATM withdrawal commit is invalid", exception);
        }
    }

    public static String fingerprint(ForeignAtmWithdrawalCommit commit) {
        try {
            return HexFormat.of().formatHex(
                    MessageDigest.getInstance("SHA-256").digest(
                            encode(Objects.requireNonNull(commit, "commit"))));
