...
public final class EscrowRuntimeManager {
    ...
    public static synchronized void tick(MinecraftServer server) {
        ...
        EscrowRuntimeService active = runtime;
        synchronized (active) {
            if (active.shouldRunRecovery()) {
                active.recoverBatch(recoveryWorkPerTick());
            }
            EscrowConfig.Settings settings = EscrowConfig.settings();
            active.tickCheckpoint(settings.checkpointIntervalSeconds(),
    ...
    /**
     * Returns the active runtime only from its owning logical server thread.
     * Lifecycle changes and value mutations share that thread, so a returned
     * runtime cannot be closed between readiness validation and use.
     */
    public static synchronized EscrowRuntimeService getOrNull(
        ...
        if (activeServer != server) {
            throw new EscrowRuntimeException(
                    "Escrow runtime belongs to another server");
        }
        return runtime;
    }
    ...
    public static synchronized EscrowRuntimeService requireReady() {
        ...
        synchronized (active) {
            if (!active.isReady()) {
                Throwable cause = active.failure().orElse(null);
                throw new EscrowRuntimeException(
