...
The `3.0.0-beta.1` implementation remains the published CurseForge and Modrinth beta baseline. The Forge source is versioned `3.0.0` after the recorded beta.3 acceptance; a stable mod platform artifact has not been uploaded. Its advanced trade offer work extends the durable 3.0 market foundation with one normalized offer contract for Server Shops and Player Shops. Focused operator documentation is available in:
...
* [External economy provider compatibility](docs/compatibility/external-economy-providers.md)
...
`forge-1.20.1/3.0.0` is the default and integration branch for this Forge release line. Pull requests,
issue fixes, verification, tags, and releases for FutureShops 3.0 target that branch. The
`neoforge-1.21.1/2.5.0` branch is a separate NeoForge line and is never an integration target
for Forge 1.20.1 changes. Older completed version branches remain available for their releases.
...
Escrow lifecycle changes and every value moving runtime operation execute on the owning logical
server thread. Server scoped runtime lookup verifies that thread, rejects a mismatched server, and
returns no runtime while shutdown is quiescing. Money claim and cash deposit flows retain the
runtime monitor across readiness checks and their dependent journal or ledger operation. Runtime
coordinator and recovery scheduler state transitions use the same monitor for each check and use.
Server Shop replay retention likewise holds the exact prepared or committed replay store monitor
while checking capacity and compacting old evidence. These paired invariants prevent runtime
shutdown, recovery transitions, or replay compaction from invalidating a decision between its
check and mutation.

Market capability requests project current server configuration, runtime readiness, module control status, claim counts, branding, currency metadata, and a display balance. The client uses the snapshot to present availability. During recovery, screens retry capability requests every second. Ready screens refresh capabilities every five seconds so an accepted module configuration change appears without reconnecting. A module disabled in `config/futureshops/futureshops-common.toml` is omitted from the marketplace header. Before the first capability response, optional navigation uses the configured flags carried by the authoritative open packet, so a disabled tab is not briefly advertised after a client cache reset. Claims only and unavailable module states are also omitted, while their durable claims remain reachable through the shared claims route. A correlated response with a newer server revision is accepted even if another retry is already outstanding, which prevents a slow response from leaving the client stuck on the recovery snapshot. Equal revision conflicts and older revisions still fail closed. Navigation remains server authoritative because a capability response can become stale immediately after it is sent. The server resolves an attempted route to the requested view, a safe fallback, or claims.
...
Protocol 58 appends the administrator bulk catalog preview, commit, and result packets. The existing searchable registry grid sends a bounded proposal to the logical server. The server resolves item identifiers against its registry, canonicalizes optional full exact NBT, parses one shared decimal price and stock value, computes deterministic rows, and returns a preview fingerprint plus expected registry and catalog fingerprints. Existing identities are skipped by default. The administrator can mark only a unique conflict for replacement. Commit rechecks permission, fingerprints, validation, capacity, and replay identity before one synchronized atomic write. A failed reload restores the complete prior catalog and runtime snapshot. Completed outcomes are retained in world SavedData for safe reconnect and restart replay.
...
Administrative entry points include `/shopadmin` and `/marketadmin`. `/marketadmin status` reports module control state, escrow runtime state, open contract counts, and pending recovery. `/marketadmin inspect <transactionId>` is a read only escrow inspection for the complete recovery handle. It reports operation and state, request identity, participants, currency provider, durable evidence phase, value, claims, retry schedule, last error, and the safe next action. `/marketadmin maintenance status` reports the persisted maintenance state and first failure. `/marketadmin maintenance verify` checks the recovery backlog, conservation, journal sequence, and global fingerprint without changing state. `/marketadmin maintenance resume confirm <reason>` submits the existing journaled `VerifyAndResume` repair only when recovery is clear and value conservation is proven. It cannot bypass an unhealthy journal or unresolved custody work. Mutating administrative operations require the configured operator level or Forge permission node, a bounded reason where configured, confirmation where configured, and an audit record.

Admin shop operators can run `/marketadmin adminshop validate` to identify each invalid listing, item, and field path. `/marketadmin adminshop quarantine_missing confirm <reason>` preserves complete listings that reference removed registry items in `config/futureshops/shops/recovery/`, atomically removes them from the active catalog, and reloads the Server Shop. Other validation errors are never removed by this command. `admin_shop.maximum_listings` in the common config defaults to 512 and accepts 1 through 10000. The catalog and network payload retain independent structural and byte limits.
...
After packaging, inspect the manifest, expanded `META-INF/mods.toml`, mixin configuration and refmap, assets, data, dependency metadata, and the complete Git diff. Version `3.0.0` must expand into the mod metadata, and `logoFile = "futureshops.png"` must resolve to the 400 by 400 project logo at the jar root. Run `verifyPackagedDependencyBoundary` and confirm the JAR contains no launcher supplied Netty, Apache Commons, Guava, Log4j, or Plexus classes and no Forge Jar in Jar metadata. Calculate candidate checksums only after the final Forge reobfuscation and packaging pass. The published `3.0.0-beta.1` build remains the public platform baseline until a stable artifact is uploaded. Build output, run directories, logs, crash reports, local configs, caches, IDE files, and `AGENTS.md` must not be committed.
...
difference when it is first opened with `3.0.0`. This is an artifact identity correction,
