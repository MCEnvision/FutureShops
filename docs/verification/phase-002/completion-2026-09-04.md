...
The exact Pixelmon `9.4.0` headless run loaded the native mixin and passed all nineteen required dedicated server GameTests. The tests now cover native cart purchase, native `/pay` transfer, native server shop sell, native admin player shop buy with claimed sale escrow, public withdraw and deposit, refund, compensation, physical money refusal, request receipt replay, durable receipt reload, unknown compound receipt recovery, and wrong entry type recovery. The native mutation test also rereads the Pixelmon save adapter file and verifies the completed receipt UUID on disk. Sanitized evidence is in [p002 integration evidence](p002-integration-evidence-2026-09-04.md). The source revision GameTest evidence SHA 256 is `8f204619413118b1598cb345f33552c89c8c25f2a08e05579649f5dd31743ddb`; the packaged server smoke log from the prior artifact remains `c0e0441699ac8e3dd72abc3348e2ae5951ceef2310e8cef593468f1ef4eb18b8`.
...
Both commands passed at the source revision above. The packaged jar passed `unzip -tq`, and its contents contain no Pixelmon, Bukkit, Vault, bridge, or external test fixture classes. The packaged exact server smoke from the prior artifact reached `Done` with the mixin target applied. The debug session also proved that production logs can discover and print the source commit and artifact SHA without a client connection. The new exact GameTest runtime stopped cleanly after the run with `eula=true` verified; its disposable world, log directory, and temporary evidence files were removed after hashing.
