...
The complete log for the current artifact is `/tmp/futureshops-vault-surface-packaged-final-20260905.log` with SHA 256 `3eb85c4e24ee3a7b66469b6796e3ac720350477731595bf85cece3a6688bd4fc`. NeoForge reported `25` registered tests and `All 25 required tests passed`. Four tests cover the enabled surface routes described above, and the companion failure matrix runs in the same exact server process. The process exited with status `0` and the disposable runtime was removed after the log and route diagnostics were recorded.
