...
The same disposable probe and rebuilt jar were loaded in the exact hybrid profile at `/tmp/futureshops-youer-pixelmon-248.zhVzs4` with `eula=true`. The bounded command was `JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 timeout 120 /usr/lib/jvm/java-21-openjdk-amd64/bin/java -Xms2G -Xmx4G -jar youer.jar nogui`. The complete log is `/tmp/futureshops-hybrid-packaged-probe-public-routes-20260903.log`. It reached `Done (26.057s)` after loading Vault `1.7.3`, FinalEconomy `1.0.9`, EverNifeCore `2.0.4.4`, and PixelmonEconomyBridge `1.1.6`. The probe again confirmed the live Pixelmon query and returned `CAPABILITY_MISSING` for both preflight and withdrawal. This demonstrates that the hybrid bridge stack cannot bypass the server capability refusal and made no external mutation.
