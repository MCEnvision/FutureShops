...
The standard dedicated server smoke reached `FutureShops common setup complete` and `FutureShops server starting` before its bounded timeout in `/tmp/futureshops-standard-server-no-pixelmon-public-routes-20260903.log` with the rebuilt artifact and Pixelmon absent. The real NeoForge GameTest server passed all thirteen required tests with Pixelmon selected in `/tmp/futureshops-pixelmon-gametest-shop-sell-20260903.log`. `pixelmonServerShopSellRefusalBeforeItemRemoval` opened the loaded default shop, submitted a valid diamond sell request, and verified that the item count and custody store were unchanged after the capability refusal. The same twelve baseline tests passed with the internal provider selected while Pixelmon was present in `/tmp/futureshops-internal-gametest-public-routes-20260903.log`.
