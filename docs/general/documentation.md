...
* `compat` contains optional integrations and must remain safe when their dependencies are absent. `compat.pixelmon` loads the exact Pixelmon 9.4.0 economy API by runtime class name and exposes query and precheck capabilities, plus the exact native account mixin when the transformed `PlayerPartyStorage` class is present.
...
`BalanceManager.clear` begins draining when the provider is ready, attempts every economy index flush even when one index throws, and writes clean markers only after the complete flush gate succeeds. Flush and marker failures are logged with the affected index and cleanup always releases the in memory handles. A missing marker or invalid checksum causes recovery on the next start. After journal recovery, any held item custody or non terminal player shop barter or sale escrow keeps the lifecycle frozen for operator recovery instead of returning to ready with an unresolved item. The server never falls back to the internal wallet during an external provider failure.

## Transaction ordering

`EconomyTransactionCoordinator` is the only typed mutation boundary. It validates lifecycle, immutable capabilities, amount, request identity, and provider precheck before writing intent. It writes `PREPARED` before creating custody, then writes `EXTERNAL_PENDING`, invokes one provider leg, validates the receipt against the request, and writes `EXTERNAL_CONFIRMED` followed by `RESOLVED`. Confirmed balance change events are published only after the resolved journal record is durable, and their reason matches the mutation kind. Journal persistence failures before or after a provider call return `RECOVERY_REQUIRED` and freeze the lifecycle. When a confirmed outcome is durable but final resolution is interrupted, the `EXTERNAL_CONFIRMED` receipt remains authoritative for recovery. Duplicate request identities replay the recorded result, while a custodied replay also verifies that the custody record reached its requested terminal state. Exceptions, missing acknowledgements, mismatched receipts, and unknown lookup outcomes become `UNCERTAIN` and freeze external mutation. Pure barter captures an exact main inventory and offhand snapshot before removing ingredients, so partial removal, stock rejection, and reward insertion failure restore the original slots without dropping or recreating variants.

The public provider contract requires balance query, precheck, withdraw, deposit, durable receipt lookup, and idempotent retry capabilities for a mutation. A provider that cannot prove a capability is refused before journal intent and before custody. A receipt resulting balance is evidence for the caller and is not copied into a FutureShops ledger. Pixelmon 9.4.0 proves query and precheck through its public API. The exact native `PlayerPartyStorage` mixin additionally proves request receipts, retry lookup, and durable saves for transformed native accounts. Custom, hybrid, and untransformed accounts remain refused. The exact integration behavior and artifact evidence are in [Pixelmon economy integration](../integrations/pixelmon-economy.md).
...
Every transaction journal transition also writes a checksummed local receipt audit record under `world/data/futureshops/receipts`. Records are written through a bounded temporary file, flushed, forced, and atomically replaced before the transition is acknowledged. The directory has its own checksummed clean marker and is loaded before provider readiness. Missing, unknown, partial, contradictory, or checksum invalid records, and any mismatch between the transaction journal and its audit records, keep the lifecycle in recovery or frozen state. This directory proves only what FutureShops recorded. It does not make a Vault operation atomic or safe to replay. The native Pixelmon mixin has a separate request receipt beside `pixelDollars`; malformed or unknown native records also force recovery. Backups must copy the local receipt directory together with the FutureShops journal, custody, claims, request identities, provider selection, and clean marker, plus the external provider's matching data.

When upgrading a world that already has a valid transaction journal but no receipt directory, the first startup backfills one current audit record for each existing request from that journal, flushes the new files, and enters recovery for the normal clean marker transition. A nonempty, malformed, or partially present receipt directory is never silently backfilled. It remains a recovery or frozen condition until its records match the transaction journal.

`PlayerShopBarterEscrowSavedData` stores the exact serialized item stacks for each player shop barter payment under `futureshops_player_shop_barter_escrow`. A payment is persisted as `PREPARED` before inventory removal, advanced to `REMOVED` only after the collected stacks match the persisted bytes, then to `STORED` after the configured barter storage accepts them, and finally to `COMPLETE` after sale delivery. Proven in process rollback uses the same exact stacks. Any non terminal record survives restart and keeps the economy in recovery until the record is resolved from evidence, so an interrupted storage write cannot be retried blindly or silently discard the buyer's items. Barter escrow bounds records, stack count, identifier lengths, and serialized stack size before persistence or reconstruction. `PlayerShopSettlementSavedData` now uses schema version 2 with a deterministic SHA 256 content checksum and clean shutdown marker, while preserving legacy version 0 and version 1 data through the normal migration path. Invalid settlement checksums, malformed rows, duplicate identities, and unknown newer schemas are read only recovery blockers.
...
Legacy `EconomyProvider` calls are presented through the coordinator backed view, so existing commands and public APIs receive fail closed behavior when a selected provider is unavailable. `ShopModAPI.queryBalance` exposes the typed result for integrations that need to distinguish confirmed, rejected, unavailable, ambiguous, and recovery required outcomes. Balance commands and administrative checks show an unavailable error instead of formatting zero, while administrative set and reset adjustments are durable coordinator legs limited to ready internal mode. Commands, shop data, dashboards, analytics, and money items obtain currency metadata through `BalanceManager` accessors rather than reading the legacy provider handle directly. Shop, dashboard, and balance leaderboard packets carry typed balance availability and the selected provider lifecycle so clients never render an unavailable balance as a confirmed zero. The balance leaderboard is available only for the internal provider until a selected external provider declares a reviewed ranking capability, and its UI labels that limitation separately from provider readiness. The player shop crash and recovery matrix now includes exact sale and barter escrow restart boundaries, offline claims, two server players, reconnect replay, and concurrent duplicate requests. Pixelmon query, precheck, and the exact native receipt path are implemented for 9.4.0, while custom and hybrid accounts fail closed. Vault remains unenabled in production until a separately installed bridge and backend prove the complete one transaction receipt and recovery contract. The repository test fixture records that contract without bundling or certifying the legacy bridge.
