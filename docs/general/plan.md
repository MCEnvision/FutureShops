...
| SRC-012 | owner_request | opt in debug mode and headless first evidence procedure | current owner request and accepted debugging workflow | binding debug toggle, structured log fields, sanitization, server log and GameTest priority, and laptop client escalation rules |
| SRC-013 | owner_request | first party interoperability boundary and DanConomy support | current owner request | binding authority to inspect and run unmodified external artifacts for interoperability, ship only original FutureShops adapter and mixin code, and add exact DanConomy support without copying, modifying, repackaging, or redistributing third party code or jars |
| SRC-014 | reference | exact DanConomy 1.2.1 artifact, public source, and ARR terms | [Modrinth version 1.2.1](https://modrinth.com/mod/danconomy/version/1.2.1), [source revision `63aecdac12e437ae1f3de2801cdea0105b3d7e06`](https://github.com/Dandelion1608/danconomy/tree/63aecdac12e437ae1f3de2801cdea0105b3d7e06), and [ARR terms](https://github.com/Dandelion1608/danconomy/blob/63aecdac12e437ae1f3de2801cdea0105b3d7e06/ARR_LICENSE.txt) | exact optional provider identity, public interface and persistence inspection, no-copy boundary, and runtime verification |
...
| External artifacts | OBSERVED | The exact Pixelmon 9.4.0 jar, interface injection metadata, DanConomy 1.2.1 jar, disposable Pixelmon runtime, and disposable hybrid runtime are available for phase-owned inspection and proof. The separately installed bridge candidate and exact hybrid stack are also available. Native Pixelmon, DanConomy saved-data, and Vault atomicity proofs remain implementation and verification gates. | EXT-001 through EXT-006 and EXT-010 are available inputs. EXT-009 records the resolved owner-approved interoperability boundary and is not an external permission blocker. |
| Public development route | VERIFIED | The officially linked MDK declares a universal jar and interface injection JSON through an artifact-only Ivy repository. The exact 9.4.0 endpoints responded HTTP 200 with lengths 400154994 and 126 bytes, the downloaded jar hash matches the recorded release hash, and an isolated Java 21 compile probe resolves the real economy and storage types on 2026-09-04. | SRC-009, exact jar and metadata hashes, archive inspection, `javap` signatures, and the isolated compile probe. This establishes development inputs, not mixin target or transaction correctness. |
...
| DanConomy transaction path | PROPOSED | Exact DanConomy 1.2.1 exposes integer minor-unit ledger balances through `EconomyAccess`, `Account`, and `LedgerData`, but its ordinary mutation methods have no request identity or durable receipt and its Pixelmon-backed currency uses a balance mirror and fallbacks. Phase 002 must add an original FutureShops mixin that stores request receipts in the same `danconomy_ledger` saved-data image as ledger balance mutations, proves durable save and retry lookup, and refuses mirrored currencies. | SRC-013, SRC-014, EXT-010, and Phase 002 implementation and runtime evidence. |
...
Missing transaction proof remains a completion gate, not permission to substitute artifacts, guess APIs, weaken transaction guarantees, or claim compatibility. Under DEC-022 and resolved EXT-009, ARR labeling does not block the in-scope work because FutureShops ships only original interoperability code and does not copy, alter, rebuild, repackage, or redistribute Pixelmon, DanConomy, Vault, or their source, bytecode, or assets. Exact unmodified artifacts may be inspected, used as compile-only inputs, and run as ordinary dependencies in disposable verification environments. This is the owner-approved project boundary, not a general license grant or permission for any operation outside it. Minecraft EULA acceptance is separately authorized and verified.
...
**Status:** RESOLVED
**Selected choice:** A public NeoForge provider API, bundled internal, optional Pixelmon and DanConomy adapters with narrow exact-version mixins, and a separate bridge registering `vault`.
**Rationale:** The core API supports exact integrations without embedding unrelated platforms. Pixelmon and DanConomy mixins are limited to reviewed exact targets and live only in FutureShops, while every external jar and the Vault bridge and backend remain separate.
...
**Status:** RESOLVED
**Selected choice:** Apply the owner's standing authorization to set and read back `eula=true` in every requested disposable Minecraft development and verification runtime. Do not request another consent phrase or treat a missing file or `eula=false` as missing consent. Apply DEC-022 and resolved EXT-009 to bounded third party inspection, compile-only use, ordinary runtime installation, and original FutureShops adapter and mixin work.
...
### DEC-021 — Headless first diagnostics and debug mode

**Status:** RESOLVED
**Selected choice:** Add an opt in, server authoritative FutureShops debug mode that is off by default, enabled by an operator at runtime with `/futureshops debug on <module>` and disabled with `/futureshops debug off`, and automatically disabled on restart. It emits bounded structured diagnostics to the server log, emits only a bounded correlated snapshot to an opted in client when client evidence is required, and never changes provider selection, readiness, transaction outcomes, or safety rules.
**Rationale:** Server logs, dedicated server GameTests, deterministic tests, and headless runtime traces provide the required evidence without requiring EnVy to join or consume laptop rendering resources. A laptop client is used only when rendering, input, client classloading, or an interaction that cannot be represented by a headless test is the acceptance gate.
**Affected requirements:** CORE-REQ-013, CORE-REQ-014, CORE-REQ-015, CORE-REQ-019, CORE-REQ-020, CORE-REQ-021
**Supersedes:** none

### DEC-022 — First party interoperability and DanConomy boundary
...
| EXT-009 | Owner-approved first party interoperability boundary | CORE-REQ-015, CORE-REQ-017, CORE-REQ-018, CORE-REQ-019, CORE-REQ-021 | available | authorized | Apply DEC-022. Inspect or decompile only lawfully obtained exact artifacts for interoperability, compile and test against unmodified artifacts, run them as ordinary dependencies, and ship only original FutureShops adapter and mixin code. Do not copy external code or assets, alter or rebuild external jars, bundle or redistribute external bytes, or extend this authorization to another operation. ARR status alone does not block work inside this boundary. |
| EXT-010 | Exact DanConomy 1.2.1 runtime and public source | CORE-REQ-015, CORE-REQ-018, CORE-REQ-019, CORE-REQ-020, CORE-REQ-021, CORE-REQ-022 | available | authorized | Use Modrinth version `9rRHHMnY`, artifact `danconomy-1.2.1.jar`, size `161611`, SHA-256 `61d3eb69a3a235929ac2376d151130e61ea4fe65c2f84990618c79e27e954b72`, SHA-512 `865aba88f26d1a78ec92b4981f9a9b5af701a5f62cb38904d9439138f7a95ac740ce80d6e717b95169e4df6e07bac3893a90ae78de8565e9c168c8b0190713f0`, and public source revision `63aecdac12e437ae1f3de2801cdea0105b3d7e06` for exact inspection, compile-only integration, and disposable runtime proof under DEC-022. Keep DanConomy unmodified and separately installed. |

### EXT-001 — Official Pixelmon 9.4.0 runtime and development artifacts

**Kind:** artifact. **Availability evidence:** The exact universal jar is present at the pinned release hash, the 126 byte interface injection metadata is present, the archive is readable, `javap` identifies the economy and storage APIs, and an isolated Java 21 compile probe resolves `BankAccount`, `BankAccountProxy`, and `PlayerPartyStorage` against the real jar. DEC-022 and resolved EXT-009 authorize the bounded inspection, compile-only, runtime, and original FutureShops mixin work. Missing Javadoc, sources, a special development classifier, or a template already pinned to 9.4.0 does not fail this prerequisite. A failed exact compile blocks the affected integration until its route is resolved; it does not restart completed phases.
...
### EXT-009 — Owner-approved first party interoperability boundary

**Kind:** authorization. **Availability evidence:** DEC-022 resolves the former permission question for the exact work in this plan. FutureShops may inspect or decompile lawfully obtained exact artifacts for interoperability, use unmodified artifacts for compile-only and disposable runtime verification, and ship original FutureShops adapter and mixin code. FutureShops may not copy external implementation code or assets, alter or rebuild external jars, bundle or redistribute external bytes, or represent external work as its own. Keep dated authoritative terms and artifact provenance in evidence, but ARR status alone is not a blocker for these bounded operations. Any newly proposed operation outside this boundary requires a separate owner decision; no unsolicited upstream message, account creation, application, purchase, third party modification, or external code publication is authorized.

### EXT-010 — Exact DanConomy 1.2.1 runtime and public source

**Kind:** artifact. **Availability evidence:** Modrinth project `UJwChUmJ`, version `9rRHHMnY`, publishes `danconomy-1.2.1.jar` for Minecraft `1.21.1` and NeoForge. The artifact is `161611` bytes with SHA-256 `61d3eb69a3a235929ac2376d151130e61ea4fe65c2f84990618c79e27e954b72` and SHA-512 `865aba88f26d1a78ec92b4981f9a9b5af701a5f62cb38904d9439138f7a95ac740ce80d6e717b95169e4df6e07bac3893a90ae78de8565e9c168c8b0190713f0`. Public source revision `63aecdac12e437ae1f3de2801cdea0105b3d7e06` declares DanConomy `1.2.1`, Minecraft `1.21.1`, NeoForge minimum `21.1.209`, integer minor-unit ledger balances, `LedgerData` saved as `danconomy_ledger`, and `PIXELMON_MIRRORED` fallback behavior. Phase 002 must verify the release bytecode against this contract before implementation. DEC-022 permits exact inspection, compile-only use, disposable execution, and original FutureShops mixin code while forbidding copied code and any modified, rebuilt, bundled, or redistributed DanConomy artifact.

### Acquisition, construction, and proof policy

Phase 000 owns initial discovery and classification. Phase 002 resumes the first unfinished acquisition or implementation task from fresh evidence without reopening completed platform or API work. P002-TASK-013 resolves exact Pixelmon compile inputs, P002-TASK-014 resolves the exact disposable hybrid stack, P002-TASK-015 verifies exact DanConomy inputs and persistence targets, and P002-TASK-006 obtains or builds the separate bridge and durable backend proof. Their missing outputs are not prerequisites to starting those tasks. Only consumption of a particular input waits for that input's review.
...
The initial hybrid acquisition candidate is PRTS `v1.21.1-1.0.30`, whose recorded installer pins NeoForge `21.1.248`, together with Vault `1.7.3`. Revalidate upstream release identity, hashes, terms, dependencies, security, and runtime compatibility. Candidate selection is not a support claim. Legacy PixelmonEconomyBridge `1.1.6`, FinalEconomy `1.0.9`, and EverNifeCore `2.0.4.4` remain separate negative or future-adaptation candidates; no modification of those artifacts is required or authorized. No unreviewed older loader or experimental parallel runtime is a silent substitute.

For DanConomy, use the exact Modrinth 1.2.1 release and public source revision recorded by EXT-010. Inspect `EconomyAccess`, `Account`, `Currency`, `CurrencyRegistry`, and `LedgerData`, then confirm the exact release bytecode before targeting it. The FutureShops adapter uses DanConomy's resolved default currency only when it is unambiguous and `LEDGER` backed. A missing or ambiguous default and every `PIXELMON_MIRRORED` currency are incompatible for this provider. The FutureShops mixin adds a request-aware extension and receipt serialization to the exact DanConomy saved-data object so the ledger balance effect and immutable receipt occupy one atomic saved-data replacement. It must force and prove durable persistence before success, deduplicate identical request IDs, reject conflicting reuse, and leave ordinary DanConomy calls unchanged. Never patch, rebuild, or redistribute the DanConomy jar.

Every acquisition attempt records source, exact target, outcome, missing technical component, boundary result, and next permitted route. Try the authoritative release, officially linked dependency route, and an already owned hash-matching artifact before declaring an external artifact unavailable. Keep network retries bounded and retry only a changed route or diagnosis. Source implementation, failed compile probes, classloader integration, schema design, and crash tests are owned work. Unavailable authenticated access or unavailable exact external bytes after these routes is an external gate. Any proposed operation outside DEC-022 must be refused, but EXT-009 is resolved for the bounded work. Ordinary plan maintenance does not assert that transaction proofs are already complete.
...
`internal` is reserved for the built in provider. `pixelmon` and `danconomy` are reserved for their exact optional FutureShops adapters. `vault` is reserved for the separately installed bridge contract. Duplicate identifiers, invalid identifiers, incompatible API versions, invalid metadata, registration outside the allowed window, or multiple ambiguous registrations must never resolve by load order. They produce a deterministic rejected registration and, when the rejected identifier is selected, an unavailable selected provider.
...
| DanConomy | Verified adapter and exact ledger saved-data mixin | Optional classes, version, currency selection, `LEDGER` versus `PIXELMON_MIRRORED` backing, save ordering, and ordinary DanConomy mutations | Inspect exact `1.2.1` source and bytecode, isolate linkage, load the mixin only when both mods are present, enable request-aware mutation only for one unambiguous ledger-backed currency with balance and receipt in the same durably replaced `danconomy_ledger` image, reject mirrored currencies, and leave ordinary DanConomy calls unchanged |
...
DanConomy compatibility is exactly DanConomy `1.2.1` on Minecraft `1.21.1` and NeoForge `21.1.248`. Its optional adapter and mixin live only in FutureShops and must not link when DanConomy is absent. The adapter resolves one unambiguous default currency and accepts only `Currency.BackingType.LEDGER`; `PIXELMON_MIRRORED` is rejected because its current reflection, balance mirror, and fallback behavior does not satisfy strict ownership or recovery. The request-aware mixin must add immutable request identity, direction, amount, account, currency, outcome, before and after values, and receipt checksum to the same `LedgerData` serialization as the balance, deduplicate identical retry, reject conflicting request reuse, and synchronously prove the `danconomy_ledger` replacement durable before reporting success. If exact target, serialization, save, or receipt lookup proof is absent, every DanConomy mutation-required surface refuses before FutureShops intent or custody. Ordinary DanConomy calls remain unchanged and outside the FutureShops transaction contract.
...
### Debug and evidence contract

Debug mode is an operator diagnostic surface, not a second transaction path. It is disabled by default, is not persisted in world data, and is not enabled by provider selection or a client request. The canonical FutureShops commands are `/futureshops debug on <module>`, `/futureshops debug off`, and `/futureshops debug status`; the implementation may expose a documented alias only when that alias is already registered by the mod. The command requires the existing operator permission boundary and accepts `all`, `provider`, `lifecycle`, `transaction`, `receipt`, `recovery`, `pixelmon`, `danconomy`, `vault`, `network`, or `surface` modules. Invalid modules and non operators receive a typed rejection and produce one bounded diagnostic.

Each enabled module writes structured, correlated records to the standard server log with a stable `futureshops.debug` category. Records include the debug session identifier, source commit and FutureShops artifact hash when known, Minecraft and loader versions, selected provider and lifecycle state, surface and operation, server owned request and leg identifiers, account classification, required and observed capabilities, validation decision, journal and receipt state, custody or claim identity, provider result or error category, elapsed time, logical side, thread, and the next safe action. Amounts may be recorded in minor units in the operator server log when needed for reproduction. Player names, IP addresses, credentials, raw NBT, inventory contents, chat, and proprietary external log payloads are never recorded. Actor identifiers in retained evidence are stable pseudonyms or request identities.

Diagnostics are event based, bounded, and rate limited. They must not log every tick, screen refresh, balance poll, or retry attempt without a state change. Stack traces are included only for an actionable failure and are sanitized before evidence retention. Client diagnostics are sent only as a bounded server correlated snapshot when the server side evidence cannot distinguish a rendering, input, client classloading, or synchronization failure. Client logs never authorize a mutation and never replace server evidence.

The debugging procedure is headless first. Run focused tests, all tests, applicable dedicated server GameTests, the complete build, and a dedicated server smoke or exact external runtime with the relevant debug module enabled. Capture the exact command, source revision, artifact hashes, host, runtime manifest, expected result, actual result, exit code, server log path, GameTest report, and sanitized diagnostic records. Use a disposable world and remove test owned processes and files after the final consumer. Connect the verified laptop client only when the acceptance criterion specifically requires rendered output, real mouse or keyboard input, client only classloading, resource presentation, or a synchronization result that the server harness cannot represent. Screenshots and recordings supplement the correlated logs and are never the primary proof when logs or GameTests can answer the question.

A debug evidence bundle is a sanitized command transcript, relevant server and optional client log excerpts, GameTest or test report, runtime and artifact manifest, and checksums. It must identify which assertions are server proven, which are client proven, and which remain unverified. Unknown, contradictory, or missing transaction evidence still follows the normal `RECOVERING` or `FROZEN` rules; debug mode cannot clear a freeze or justify a retry.
...
**Behavior:** Emit concise structured diagnostics for provider selection, compatibility, readiness, state transitions, rejected mutation categories, ambiguous outcomes, recovery, and compensation. Provide an opt in server authoritative debug mode with `/futureshops debug on <module>`, `/futureshops debug off`, and `/futureshops debug status`, structured request correlation, bounded module selection, rate limits, sanitized operator logs, and optional bounded client snapshots for client only evidence. Provide operator visible status without exposing secrets or unnecessary private balance data. Avoid repeated hot path log spam and never let debug mode change transaction behavior.
**Owner:** `CORE-PHASE-001`
**Contributors:** `CORE-PHASE-002`, `CORE-PHASE-003`
**Dependencies:** CORE-REQ-004, CORE-REQ-007, DEC-021
**Lifecycle stage:** continuous
**Production verification:** nondestructive
**Release impact:** stable release
...
- An operator can identify selected provider, state, safe next action, and correlated request without reading private data; debug mode is off by default and can be enabled or disabled by an authorized operator; selected modules emit the documented fields and no secrets or raw private data; repeated rejected ticks or screen refreshes do not spam logs; exceptions retain actionable context.
...
**Behavior:** Keep provider lookup outside hot paths, use the frozen selected service, avoid filesystem or network access during gameplay mutation, avoid full balance scans and balance shadow synchronization, require bounded provider calls on the server execution model, and bound debug event volume, retention, and client snapshots.
**Owner:** `CORE-PHASE-001`
**Contributors:** `CORE-PHASE-002`, `CORE-PHASE-003`
**Dependencies:** CORE-REQ-002, CORE-REQ-004, CORE-REQ-009, DEC-014, DEC-021
**Lifecycle stage:** continuous
**Production verification:** nondestructive
**Release impact:** stable release
...
- No per tick provider discovery, disk scan, registry scan, external balance synchronization, or debug log flood exists; ordinary requests do work proportional to their transaction legs; an unavailable provider does not create a retry storm; debug status and module changes do not mutate economy state.
...
**Behavior:** Validate all API, packet, command, config, metadata, value, receipt, debug-module, and persistence inputs. Contain provider exceptions. Preserve server permissions and identity. Sanitize and rate limit debug output, prohibit secrets, private raw player data, credentials, IP addresses, raw NBT, and proprietary external payloads, and keep client diagnostics non authoritative. Review every external artifact for provenance, hashes, license, archive contents, dependency risks, and known security concerns before use. Keep optional implementation classes isolated and enforce DEC-022's no-copy, no-modified-jar, no-bundling, and no-redistribution boundary.
**Owner:** `CORE-PHASE-001`
**Contributors:** `CORE-PHASE-002`, `CORE-PHASE-003`
**Dependencies:** CORE-REQ-002, CORE-REQ-004, CORE-REQ-005, CORE-REQ-007, DEC-021, EXT-001, EXT-004, EXT-005
**Lifecycle stage:** continuous
**Production verification:** nondestructive
**Release impact:** stable release
...
- No untrusted value bypasses validation; no Bukkit or Vault dependency or reflection exists in FutureShops; Pixelmon or DanConomy absence cannot cause class loading failure; no credential or private raw log enters artifacts or evidence; unresolved artifact risk blocks the related integration.
...
**Dependencies:** CORE-REQ-001, CORE-REQ-002, CORE-REQ-004, CORE-REQ-006, CORE-REQ-007, CORE-REQ-009, CORE-REQ-015, CORE-REQ-016, DEC-017, DEC-018, DEC-019, DEC-020, DEC-022, EXT-001, EXT-002, EXT-003, EXT-008, EXT-009
**Lifecycle stage:** change
**Production verification:** nondestructive
**Release impact:** stable release
...
### CORE-REQ-018 — DanConomy and separate Vault interoperability

**Behavior:** Bundle original FutureShops adapter and mixin code for exactly DanConomy `1.2.1` without bundling or modifying DanConomy. Register provider identifier `danconomy`, resolve one unambiguous default currency, and accept only `LEDGER` backing. Inspect exact `EconomyAccess`, `Account`, `Currency`, `CurrencyRegistry`, and `LedgerData` source and release bytecode before targeting them. Add a request-aware ledger entrypoint and receipt serialization through a narrow FutureShops mixin so each accepted balance mutation and immutable receipt are written in the same `danconomy_ledger` saved-data image, identical retries return the stored outcome, conflicting request reuse is rejected, and success is returned only after a proven durable save or atomic replacement. Refuse `PIXELMON_MIRRORED` currencies and every unproven save path before FutureShops intent or custody. Leave ordinary DanConomy calls unchanged. Also support a separately installed and reviewed hybrid bridge that registers provider identifier `vault` through the public API. In Phase 002, inspect the bridge and economy backend boundary and execute a disposable transaction proof that persists the balance effect and provider receipt in one transaction, injects interruption before and after commit, looks up the original request, and proves duplicate retry behavior. Validate its declared query, precheck, withdraw, deposit, durable receipt lookup, and idempotent retry capabilities against one exact hybrid runtime, Vault artifact, and economy plugin stack. The currently observed PixelmonEconomyBridge and FinalEconomy path does not satisfy that contract without modification, so that legacy stack remains safely refused. Under DEC-019, Phase 002 acquires a reviewed existing registrant or builds a separate test registrant and durable backend; its implementation is not an externally supplied prerequisite. FutureShops records each local request and returned provider outcome in the durable receipt audit journal under `world/data/futureshops/receipts`, but that record does not make a Vault boolean call atomic or safe to replay. Enable only surfaces whose capabilities are proven; otherwise fail closed with no generic Vault production support claim. Keep all external code and artifacts outside FutureShops except original FutureShops adapter and mixin classes. Production transaction-aware bridge and backend adaptation plus the DanConomy port obligations are issue 66 scope for the 3.0.0 Forge `1.20.1` beta and future `1.21.1` port.
**Owner:** `CORE-PHASE-002`
**Contributors:** `CORE-PHASE-000`, `CORE-PHASE-001`, `CORE-PHASE-003`
**Dependencies:** CORE-REQ-002, CORE-REQ-004, CORE-REQ-007, CORE-REQ-009, CORE-REQ-015, CORE-REQ-016, DEC-017, DEC-019, DEC-020, DEC-022, EXT-004, EXT-005, EXT-006, EXT-008, EXT-009, EXT-010
**Lifecycle stage:** change
**Production verification:** nondestructive
**Release impact:** stable release
...
- `danconomy` resolves only through the exact FutureShops 1.2.1 adapter and mixin; a ledger-backed currency proves one saved-data balance and receipt image, synchronous durability, lookup, duplicate retry, conflict rejection, crash recovery, absence isolation, and unchanged ordinary DanConomy behavior; mirrored currencies refuse before effects; no DanConomy byte or copied code appears in FutureShops. `vault` resolves only through the separate bridge; Phase 002 produces a transaction-proof result for the bridge or backend; every enabled mutation surface proves the exact capability set, one-transaction provider receipt contract, and crash recovery contract; the current unmodified hybrid stack remains refused; a missing, failed, or capability-incomplete stack leaves the server online and FutureShops fail closed; no Bukkit or Vault class, reflection string, dependency, service lookup, or bundled bridge appears in FutureShops; exact limitations are documented without claiming generic compatibility.

**Required evidence**

- `EVD-EXT-005` through `EVD-EXT-008`, exact DanConomy source and bytecode map, saved-data receipt and crash matrix, ordinary-call and mirrored-currency refusal tests, bridge and backend transaction-proof fixture or exact implementation trace, interrupted-commit and retry lookup results, integration logs, dependency and jar scans, registration tests, and environment manifests.

### CORE-REQ-019 — complete production validation

**Behavior:** Execute the complete deterministic and runtime matrix after all implementation and phase-owned documentation are integrated. Prove clean draining, receipt audit journal integrity under `world/data/futureshops/receipts`, unclean-start recovery, unknown-record classification, frozen ambiguity, escrow and claim conservation, no unsafe retries or balance restoration, exact provider capability gating for Pixelmon, DanConomy, and Vault, and the headless first debug evidence procedure. Resolve failures through the owning phase requirement, rerun invalidated checks, and preserve a sanitized debug evidence bundle tied to the source commit and exact external artifacts.
**Owner:** `CORE-PHASE-003`
**Contributors:** `CORE-PHASE-000`, `CORE-PHASE-001`, `CORE-PHASE-002`
**Dependencies:** CORE-REQ-001, CORE-REQ-002, CORE-REQ-003, CORE-REQ-004, CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-008, CORE-REQ-009, CORE-REQ-010, CORE-REQ-011, CORE-REQ-012, CORE-REQ-013, CORE-REQ-014, CORE-REQ-015, CORE-REQ-016, CORE-REQ-017, CORE-REQ-018, DEC-021, DEC-022, EXT-001, EXT-002, EXT-003, EXT-004, EXT-005, EXT-006, EXT-010
...
- Every mandatory requirement has passing evidence; no required command fails; external environments use exact reviewed artifacts; no unverified behavior is described as working; headless tests, server logs, and GameTest reports are the primary evidence; any laptop client run is justified by a client only acceptance criterion and is correlated to the server evidence; results are reproducible from the recorded commit and manifests.
...
**Behavior:** Update the root user documentation, maintainer documentation, documentation index, provider API reference, configuration guide, integration guides, migration guide, recovery runbook, compatibility matrix, debug and support runbook, and validation record to match implemented behavior only, including strict capabilities, lifecycle states, journal and escrow ordering, the `world/data/futureshops/receipts` receipt audit journal, clean marker, backup contents, unknown-record recovery, frozen operator workflow, the native Pixelmon receipt and durable-save contract, direct Pixelmon's mutation limitation, the exact DanConomy ledger receipt and durable-save contract, DanConomy mirrored-currency refusal, the separate transaction-aware Vault bridge boundary, the DEC-022 no-copy and no-redistribution boundary, the opt in debug commands and module fields, sanitization and rate limits, headless GameTest procedures, and the client escalation gate.
**Owner:** `CORE-PHASE-003`
**Contributors:** `CORE-PHASE-000`, `CORE-PHASE-001`, `CORE-PHASE-002`
**Dependencies:** CORE-REQ-001, CORE-REQ-002, CORE-REQ-003, CORE-REQ-004, CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-008, CORE-REQ-009, CORE-REQ-010, CORE-REQ-011, CORE-REQ-012, CORE-REQ-013, CORE-REQ-014, CORE-REQ-015, CORE-REQ-016, CORE-REQ-017, CORE-REQ-018, CORE-REQ-019, DEC-021, DEC-022, EXT-010
**Lifecycle stage:** post_change
**Production verification:** nondestructive
**Release impact:** stable release
...
- A user can select `internal`, `pixelmon`, `danconomy`, or the separately installed `vault` provider correctly and can tell whether each surface is capability-enabled or safely refused; an integrator can implement the public API; an operator can enable debug mode, select a module, capture a sanitized correlated server log and GameTest evidence bundle, and disable it without changing economy state; an operator can drain, back up, diagnose, recover, or preserve a frozen request and an unknown receipt record without deleting data or restoring external balances; documentation clearly states the receipt audit path, its local-evidence limit, native Pixelmon receipt and durable-save rules, exact DanConomy ledger receipt rules and mirrored-currency refusal, transaction-aware Vault bridge requirement, unknown-record recovery, no migration, restart-only selection, direct API limitation, external money item behavior, exact compatibility, and no ATM or publication.
...
**Owner:** `CORE-PHASE-003`
**Contributors:** `CORE-PHASE-000`, `CORE-PHASE-001`, `CORE-PHASE-002`
**Dependencies:** CORE-REQ-019, CORE-REQ-020, DEC-016, DEC-021, DEC-022, EXT-010
**Lifecycle stage:** post_change
**Production verification:** nondestructive
**Release impact:** stable release
...
| --- | --- | --- | --- | --- | --- |
| `CORE-PHASE-000` | [`phases/plan-phase-000.md`](phases/plan-phase-000.md) | Verified external prerequisites, pinned platform, frozen capability-aware public API, restart-only selection contract, and debug command contract | `CORE-REQ-001`, `CORE-REQ-002`, `CORE-REQ-003` | Master and complete registered plan set accepted, repository baseline reconfirmed | `EXT-001` through `EXT-006` and `EXT-010` are classified with exact evidence, EXT-008 applies standing DEC-020 authorization, EXT-009 applies the resolved DEC-022 interoperability boundary, the direct Pixelmon and DanConomy APIs' missing mutation capabilities produce frozen safe-refusal contracts, the debug command and evidence fields are frozen, and the platform and public API acceptance gates pass |
| `CORE-PHASE-001` | [`phases/plan-phase-001.md`](phases/plan-phase-001.md) | Complete strict economy gate, lifecycle draining and recovery, write-ahead journal, escrow and claims, capability routing, migration safety, presentation, bounded debug diagnostics, backup, and frozen recovery | `CORE-REQ-004`, `CORE-REQ-005`, `CORE-REQ-006`, `CORE-REQ-007`, `CORE-REQ-008`, `CORE-REQ-009`, `CORE-REQ-010`, `CORE-REQ-011`, `CORE-REQ-012`, `CORE-REQ-013`, `CORE-REQ-014`, `CORE-REQ-015`, `CORE-REQ-016` | `CORE-PHASE-000` integrated and its public contract stable | Every owned requirement passes deterministic, persistence, clean and unclean shutdown, escrow conservation, failure, server, client, multiplayer, backup, recovery, frozen-state, and headless debug evidence gates with internal and fixture external providers |
| `CORE-PHASE-002` | [`phases/plan-phase-002.md`](phases/plan-phase-002.md) | Native-account capability-gated Pixelmon `9.4.0`, exact ledger-backed DanConomy `1.2.1`, separate `vault` bridge interoperability, and integration debug traces | `CORE-REQ-017`, `CORE-REQ-018` | `CORE-PHASE-001` integrated; acquisition and construction start in P002-TASK-001, P002-TASK-013, P002-TASK-014, P002-TASK-015, and P002-TASK-006 without requiring their outputs at entry; reviewed inputs and resolved EXT-008 through EXT-010 apply before dependent use | The native Pixelmon account path and DanConomy ledger path prove request receipts, retry deduplication, durable saves, exact account or currency classification, and correlated debug traces or refuse safely; DanConomy mirrored currency refuses; the separate Vault stack proves one-transaction provider receipts or remains refused; all exact stacks pass lifecycle, journal, escrow, restart, recovery, isolation, packaging, and headless diagnostics matrices; standard NeoForge remains clean |
| `CORE-PHASE-003` | [`phases/plan-phase-003.md`](phases/plan-phase-003.md) | Final strict-safety validation, headless-first debug evidence audit, accurate documentation, validated unpublished artifact, and post-artifact verification and update of open issue 66 | `CORE-REQ-019`, `CORE-REQ-020`, `CORE-REQ-021`, `CORE-REQ-022` | `CORE-PHASE-002` integrated with complete external evidence and issue 66 identified by the authoring `EVD-GH-001` record | Plan-wide definition of done passes with server logs and GameTest evidence primary, client runs justified by client-only gates, exact artifact remains unpublished, and issue 66 is updated only after artifact validation with the 3.0.0 design, verified by URL and readback, and remains open |
...
10. Run restart and crash point recovery matrices with internal, fixture external, native Pixelmon, exact DanConomy, and `vault` providers.
11. Run the exact Pixelmon environment, exact DanConomy environment, and exact hybrid bridge environment.
...
### Headless first debug evidence procedure

Use the smallest evidence source that can prove the requirement. Run focused tests, all tests, applicable dedicated server GameTests, the complete build, and server smoke or exact external runtime checks first. Enable the relevant debug module only for the bounded reproduction window. Prefer structured server log records and GameTest assertions over screenshots. Each record and retained bundle must include the source revision, FutureShops artifact hash, exact external hashes when relevant, host, Java and loader versions, command or test name, expected and actual result, exit code, lifecycle and provider state, request and leg identities, capability and validation result, journal and receipt state, custody or claim state, error category, and sanitized log or report path.

Use a laptop client only when the acceptance criterion requires rendering, real input, client-only classloading, resource presentation, or a synchronization result that cannot be represented by a dedicated server or headless client harness. Correlate the client log to the server session and request identity. Do not treat client screenshots, a human joining, or a client-side balance as proof of server authority or external transaction safety. Stop owned processes, retain only the sanitized bundle, and remove disposable worlds, logs, reports, and runtime output after their final consumer.
...
| DanConomy `1.2.1` | Absent, exact ledger-backed default currency, missing or ambiguous default, `PIXELMON_MIRRORED` refusal, incompatible version, startup failure, runtime failure, receipt in the same saved-data image, durable replacement, conflicting request, duplicate and crash recovery |
...
The final jar is scanned for Pixelmon and DanConomy artifacts, copied third party implementation code, Bukkit and Vault classes, bridge classes, compile time Bukkit or Vault dependencies, reflection strings targeting Bukkit or Vault, accidental embedded jars, credentials, private paths, caches, logs, test worlds, and debug output. Original FutureShops Pixelmon and DanConomy adapter and mixin classes are expected, but all external runtime and development artifacts are excluded.

#### Debug diagnostics matrix

| Debug case | Required evidence |
| --- | --- |
| Debug disabled | Startup and command status show debug is off, no diagnostic flood occurs, and transaction behavior is unchanged. |
| Module enabled | Authorized operator enables one module, records the session identifier, reproduces one bounded operation, and receives structured server log records with the required correlation and state fields. |
| Invalid or unauthorized toggle | Invalid module and non operator commands are rejected without changing state and produce one sanitized category record. |
| Recovery and ambiguity | Journal, provider receipt, custody, claim, `RECOVERING`, and `FROZEN` diagnostics explain the next safe action without clearing a freeze or authorizing a retry. |
| Client escalation | A client run exists only for a client-only acceptance criterion and its log is correlated to the server evidence; otherwise the row is explicitly marked not needed. |

### Evidence quality

Evidence must identify the source commit, exact command or procedure, date, host, environment manifest, relevant artifact hashes, debug session and request identities when applicable, expected result, actual result, exit code, and sanitized log or report location. A passed test against an unrecorded external artifact is not valid production evidence. Server logs and dedicated GameTests are primary evidence for server behavior. Manual evidence, screenshots, and recordings supplement but never replace deterministic checks or a correlated server record.
...
* Exact DanConomy `1.2.1` installation, default ledger-currency selection, request-receipt saved-data mixin, durable replacement, `PIXELMON_MIRRORED` refusal, ordinary-call boundary, and the fact that DanConomy itself is not bundled or modified.
...
* Operator status, opt in debug commands and module selection, structured server and optional client logs, sanitization and rate limits, headless GameTest procedure, the durable receipt audit journal at `world/data/futureshops/receipts`, backup, provider failure, unknown-record recovery, recovery required, restore, and selection rollback procedures.
...
| --- | --- | --- | --- | --- |
| `RISK-001` | Exact Pixelmon compile inputs become unavailable or fail exact verification | Adapter compilation or runtime mixin verification cannot be completed | Execute P000-TASK-002 and P002-TASK-013, verify the universal jar and interface injection recipe, and record the failed probe or exact missing input | Blocks affected compile or runtime proof after the acquisition paths are exhausted; ARR status, missing sources, or an older MDK template alone is not a blocker under DEC-022 |
...
| `RISK-005` | Crash occurs between an external effect and local outcome persistence | Duplicate or lost value may occur | Require durable provider outcome lookup, a proven native Pixelmon or DanConomy receipt, or a Vault backend transaction, and enter recovery required on ambiguity | Blocks `CORE-REQ-007` |
...
| `RISK-013` | DanConomy ledger mutation and request receipt do not reach one durable saved-data image, or a mirrored Pixelmon currency is accepted | A crash can leave an unknowable outcome or a second ledger can become authoritative | Target exact DanConomy `1.2.1` only, persist balance and receipt in the same `danconomy_ledger` serialization, force and prove durable replacement, reject ambiguous defaults and every `PIXELMON_MIRRORED` currency, and leave ordinary DanConomy calls unchanged | Blocks DanConomy mutation support under `CORE-REQ-018`; safe refusal remains required |
...
13. The bundled original FutureShops DanConomy adapter registers `danconomy` only for exact DanConomy `1.2.1`, accepts only one unambiguous ledger-backed default currency, passes same-saved-data balance and receipt persistence, durable replacement, lookup, retry, conflict, crash, and recovery proof, refuses mirrored currencies, leaves ordinary DanConomy calls unchanged, and contains no copied or bundled DanConomy bytes. The separately installed bridge registers `vault` and passes the Phase 002 backend transaction proof plus capability, one-transaction receipt, enablement or safe-refusal, journal, escrow, restart, and recovery checks against one exact reviewed hybrid stack, while FutureShops contains no Bukkit or Vault dependency, reflection, or bridge code. The current PixelmonEconomyBridge and FinalEconomy path remains refused unless separately modified outside this plan to satisfy that contract.
