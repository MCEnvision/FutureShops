...
| External artifacts | UNKNOWN | Exact official Pixelmon provenance, bridge, hybrid stack, license, security, and disposable environment evidence remains unresolved. | EXT-001 through EXT-006 require recorded hashes, manifests, reviews, and runtime proof. |
...
**Rationale:** The bridge owns hybrid-platform dependencies and lifecycle adaptation. Phase 002 owns the proof boundary and must produce a disposable transaction proof or a typed safe-refusal result. The currently observed PixelmonEconomyBridge and FinalEconomy path does not provide that atomic request-aware contract without modification, so production enablement remains refused and the modification is tracked for 3.0.0 in issue 66.
...
### EXT-008 — Owner acceptance of applicable EULA and terms for disposable runtime validation

**Kind:** authorization. **Required evidence:** Owner-provided acceptance for the exact disposable Pixelmon and hybrid runtime terms, recorded without credentials or private account data, before changing any environment from `eula=false` or equivalent. Until supplied, environment assembly and read-only artifact inspection may continue, but full runtime mutation, restart, and recovery evidence remains blocked.
...
Official Pixelmon 9.4.0 runtime and development artifacts, Disposable exact Pixelmon 9.4.0 integration environment, Separately installed Vault bridge artifact, Exact reviewed hybrid runtime, Vault, and economy plugin stack, Disposable exact Vault bridge integration environment, and Owner acceptance of applicable EULA and terms for disposable runtime validation under EXT-008 are known external blockers. The native Pixelmon mixin and the in-phase Vault backend transaction proof are implementation work in CORE-PHASE-002, not prerequisites to begin that phase. If any external runtime item remains unavailable, dependent runtime scope is preserved and the result is **NOT COMPLETE — EXTERNALLY BLOCKED**. A negative capability result requires safe refusal rather than a false support claim. No substitute artifact, reduced verification, unsafe override, partial compatibility claim, or publication may bypass that state.
