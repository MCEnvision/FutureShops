...
| VERIFIED | Strict economy gate | The integrated core declares capabilities explicitly, journals intent before permitted external effects, preserves item custody and claims, drains on orderly shutdown, recovers before writes after an unclean shutdown, and freezes unresolved uncertainty. | `DEC-017`, `CORE-REQ-002`, `CORE-REQ-004`, `CORE-REQ-005`, `CORE-REQ-007`, `CORE-REQ-009`, and `CORE-REQ-016` completion evidence | Any API, lifecycle, journal, escrow, claim, or recovery change invalidates both adapter integration proofs. |
...
- `CORE-REQ-018`: exercise the strict economy gate, journal, escrow, claims, orderly draining, unclean-start recovery, frozen uncertainty, duplicate request handling, service loss, restart, reconnect, and multiplayer behavior against the exact reviewed stack.
...
8. `P002-TASK-008` exercises all CORE-REQ-018 enabled `vault` surfaces through journal, escrow, claims, draining, recovery, frozen ambiguity, duplicate, restart, and service-loss workflows.
...
- Exact `vault` bridge registration, capability, conversion, mutation, receipt, idempotency, duplicate, crash, recovery, restart, service-loss, multiplayer, and surface matrices.
...
| Journal, escrow, custody, and claims | `CORE-PHASE-001` | Intent-before-permitted-effect, durable custody, claim conservation, provider binding, and uncertain outcome preservation | Persistence and crash matrices | Freeze affected mutation path; never guess, retry blindly, or restore an external balance from local data. |
...
| `P002-TASK-008` | CORE-REQ-018 | Exercise each enabled `vault` surface through write-ahead intent, item custody, external effect, durable outcome, delivery or claim, draining, restart, recovery, and frozen ambiguity. | P002-TASK-007, Phase 001 journal and escrow | Exact surface and recovery matrix | All monetary surfaces, journal, escrow, claims, lifecycle | Injected failures at every boundary, crash and restart, multiplayer, reconnect, conservation assertions |
...
FutureShops does not implement a Vault adapter. The separately installed bridge registers `vault` through the same public NeoForge provider contract used by any external registrant. FutureShops contains no Bukkit or Vault imports, dependencies, reflection, service lookup, plugin lifecycle code, or bridge special case. The bridge owns translation to the exact hybrid runtime and economy plugin. FutureShops accepts its mutation capabilities only after runtime proof demonstrates stable request identity, durable receipt lookup, idempotent retry, exact value conversion, and typed failure behavior.

For an enabled `vault` mutation, the Phase 001 state machine remains authoritative: persist intent and custody before the external effect, use stable root and leg identities, persist a proven external outcome before delivery, and turn failed delivery into a durable claim. `DRAINING` rejects new work and lets bounded in-flight work settle before a clean marker. An unclean start enters `RECOVERING`; receipt lookup by stable identity decides a proven outcome. Missing or contradictory evidence enters `FROZEN`; no blind retry, fabricated refund, automatic external balance restore, or internal fallback is permitted.
...
| Bridge returns ambiguous mutation result | Missing receipt, conflicting receipt, timeout, or disconnect after effect boundary | Preserve journal and custody, enter recovery or frozen state, do not retry blindly | Query durable receipt by stable identity; operator intervention if still unknowable | Injected timeout and crash matrix |
...
| Unclean restart occurs | Clean marker absent or in-flight intent remains | Enter `RECOVERING` before admitting writes; reconcile enabled `vault` requests by durable receipt; direct Pixelmon has no mutation entries because refusal was pre-intent | Resume only after all outcomes are proven; otherwise remain `FROZEN` | Crash at each state transition and recovery inspection |
...
| `P002-TASK-008` | Journal and state transition assertions | All enabled surfaces through escrow and claims | Multiplayer, restart, reconnect, shutdown, crash, and recovery | Interrupt every state and external-effect boundary | Surface, conservation, and recovery matrix |
...
| `CORE-REQ-018` | API boundary, capability, conversion, receipt, replay, and isolation suites | Complete exact `vault` surface mapping | Exact hybrid multiplayer, restart, service loss, crash, and recovery | Every component absent, incomplete capability, ambiguity, duplicate | `EVD-EXT-005` through `EVD-EXT-007`, matrices, logs, dependency scan |
...
Backup and recovery procedures preserve a complete matching world, journal, custody, claims, internal data, and external provider data. They must never instruct operators to delete transaction evidence, infer an external outcome from a local balance snapshot, retry an uncertain mutation blindly, or edit a balance as guessed compensation. Operator rehearsals use disposable copies and demonstrate exact installation, provider selection and restart, safe Pixelmon refusal, exact `vault` transaction and recovery when proven, dependency loss, claim preservation, complete backup restoration, and return to `internal` without balance transfer.
...
| External effect is ambiguous after crash | Intent first, stable identity, durable receipt lookup, frozen fallback | Incomplete or contradictory outcome after restart | Reconcile by receipt or remain `FROZEN`; never blind retry | Affected mutation, surface, crash, and recovery evidence | Earliest failed boundary through `P002-TASK-008` completion |
...
8. A complete exact `vault` capability, conversion, lifecycle, request, receipt, idempotency, duplicate, journal, escrow, claim, shutdown, crash, recovery, frozen, and surface matrix against `EXT-004` through `EXT-006`.
