...
| VERIFIED | Continuation issue | Plan authoring created and read back open issue 66 after plan validation. This phase preserves its recorded identity without modifying, duplicating, or closing it. | `DEC-015`, `EXT-007`, `EVD-GH-001` authoring evidence | Phase 003 performs the required product-lifecycle readback only after artifact validation. |
...
- `CORE-REQ-019` through `CORE-REQ-021` and Phase 003 verification and update work for `CORE-REQ-022` belong to `CORE-PHASE-003`. Issue 66 already exists from validated plan authoring and remains open and unmodified in Phase 001.
