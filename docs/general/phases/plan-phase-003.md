...
| VERIFIED | Runtime terms authority | EXT-008 is available and authorized by DEC-020. Each requested disposable runtime records `eula=true` readback before launch and separate EXT-009 review. | Phase 002 completion packet, standing DEC-020 authorization, and exact runtime readback | Changed artifacts or configuration invalidate affected runtime proof, not standing Minecraft EULA authorization. Recheck changed third party terms under EXT-009. |
...
Phase entry stops if Phase 002 is not integrated, exact external environments cannot be reproduced, the authorized EXT-008 runtime file was not verified or a required EXT-009 permission remains unresolved, the authoring evidence does not identify issue 66, or a prior mandatory requirement lacks evidence. Apply existing DEC-020 authorization without asking again. No later task may infer unrelated permissions or convert missing runtime evidence into a documentation caveat.
...
- The final audit reruns `/futureshops debug on <module>`, `/futureshops debug off`, and `/futureshops debug status` against the shared `all`, `provider`, `lifecycle`, `transaction`, `receipt`, `recovery`, `pixelmon`, `vault`, `network`, and `surface` module allowlist. It proves bounded sanitized server records first and records a client run only when a client-only acceptance criterion requires it.
...
- Prior evidence records EXT-008 and EXT-009 for the exact inputs. When recreating requested disposable runtimes, set and verify `eula=true` under DEC-020 without renewed consent. Do not infer permission for unrelated third party operations.
...
5. `P003-TASK-005` executes CORE-REQ-019 through internal, fixture, native Pixelmon, and `vault` capability, failure, crash, idempotency, recovery, and surface matrices, including native receipts beside `pixelDollars`, receipt audit records under `world/data/futureshops/receipts`, their matching backup and restore, unknown-record classification, custom and hybrid Pixelmon refusal, direct Pixelmon refusal before intent or custody where durable provider receipts and idempotent retry are absent, and correlated `pixelmon`, `vault`, `receipt`, and `recovery` debug records.
...
| Runtime authorization and terms | `EXT-008`, `EXT-009`, `DEC-020` | Standing Minecraft authorization is applied and necessary third party operations are permitted | Compare exact runtime file readback, environment identities, launch chronology, and terms review | Correct a missing or false `eula.txt` under DEC-020. Only an unresolved separate permission requires owner direction; do not invent runtime evidence. |
...
| `P003-TASK-005` | `CORE-REQ-019`, `DEC-017`, `DEC-018`, `DEC-021`, `EXT-008` | Execute provider, surface, clean-stop, crash-point, retry, compensation, claim, backup, restore, and operator-reconciliation matrices. Prove native Pixelmon request receipts, retry deduplication, durable saves, custom and hybrid account refusal, direct-call refusal before intent, custody, or provider effect, and correlated integration diagnostics. Prove `vault` mutation only when the bridge and backend persist one balance and receipt transaction; otherwise record safe refusal with the relevant debug fields. | Exact external manifests, owner-authorized disposable launches, stable request fixtures | `EVD-VER-002` and sanitized debug bundle | Strict economy gate, lifecycle coordinator, journal, custody, claims, Pixelmon adapter, external bridge, diagnostics | No duplicate effect, guessed outcome, fallback, external balance restore, loss, unsafe mutation, sensitive log, or unjustified client run; uncertainty remains `FROZEN`. |
...
| Runtime file or separate terms review incomplete | Missing `eula=true` readback or exact unresolved EXT-009 operation | Set and read back the runtime file under DEC-020; do not infer unrelated permission or passing evidence. | Resolve only the actual external permission gap and recreate the affected run with reviewed inputs. | Readback, terms and environment audit, then the full affected runtime matrix. |
...
| Issue guidance omits a strict design topic or blurs the version boundary | Post-update checklist or diff finds missing gate, state, journal, receipt audit path, escrow, capability, Pixelmon, bridge, backup, unknown-record recovery, reconciliation, or port guidance | Treat `CORE-REQ-022` as failed; do not close the issue or completion packet. | Correct issue 66 only, read it back, and repeat the no-code and metadata audit. | Complete issue guidance checklist and exact remote body readback. |
...
Documentation must state `internal` default, restart only selection, no fallback, exact minor units, no automatic migration, external money item restrictions, no ATM, the central gate, intent-before-effect journaling, the durable receipt audit path `world/data/futureshops/receipts`, its local-evidence limit, buy and sell custody ordering, durable claims, `READY`, `DRAINING`, `RECOVERING`, and `FROZEN`, unknown-record handling, provider capability requirements, the native Pixelmon request receipt, deduplication, durable-save, and account-classification rules, the direct Pixelmon API's no-mutation classification, the separate transaction-aware bridge boundary, checksummed backup contents, operator reconciliation, exact Pixelmon and hybrid limits, optional dependency isolation, candidate hashes, and unpublished status. Rehearse drain, unclean restart, unknown receipt records, backup, frozen preservation, provider-receipt-based recovery, and restore procedures on disposable copies with verified `eula=true` under EXT-008 and DEC-020 and separate third party operations reviewed under EXT-009.
...
| Third party rights confused with Minecraft EULA authorization | Apply DEC-020 and review each necessary external operation under EXT-009. | Unresolved third party right or absent runtime readback | Correct the runtime configuration; resolve only the separate right that remains uncertain. | Affected Pixelmon, hybrid, artifact reinstall, and terminal evidence | Repeat invalidated workflows after the exact configuration or permission gap is resolved. |
...
Any failed command, stale evidence, hash mismatch, unverified exact stack, missing EXT-008 runtime readback or unresolved required EXT-009 permission, missing recovery proof, documentation discrepancy, forbidden jar content, unsafe direct Pixelmon mutation, sensitive evidence, inaccessible or incorrect issue, incomplete issue guidance, `3.0.0` code change, premature issue closure, or publication blocks completion.
