...
7. `P003-TASK-007` executes CORE-REQ-020 by reconciling and rehearsing all required documentation against the passing implementation and environments, including local receipt evidence limits, unknown-record recovery, matching backup contents, and frozen operator handling.
...
Under DEC-019, document the exact Pixelmon universal-jar and interface injection acquisition and compile procedure, reviewed permissions, and the separate bridge and backend build recipe. Distinguish an exact proof backend's successful transaction evidence from the refused unmodified PixelmonEconomyBridge and FinalEconomy stack. After artifact validation, P003-TASK-011 carries these acquisition sources and the request-aware extension, shared authoritative ledger, immutable receipt schema, one-transaction commit, classloader contract, and crash-test obligations into the existing issue 66 guidance for both 3.0.0 targets. Do not describe the proof components as a published production bridge or infer support for another backend.
...
11. A completed issue-guidance checklist proving issue 66 explains one economy gate, write-ahead identity and states, the durable receipt audit path `world/data/futureshops/receipts` and its local-evidence limit, clean marker, unknown-record `RECOVERING` or `FROZEN` handling, buy and sell custody ordering, durable claims, `READY`, `DRAINING`, `RECOVERING`, `FROZEN`, provider capability negotiation, the native Pixelmon `PlayerPartyStorage` receipt, deduplication, durable-save, and account-classification pattern, direct Pixelmon no-mutation behavior outside that path, the separate transaction-aware Vault bridge or backend contract, checksummed backup scope, evidence-based operator reconciliation, Forge `1.20.1` implementation work, and future `1.21.1` port revalidation.
