...
5. `P003-TASK-005` executes CORE-REQ-019 through internal, fixture, Pixelmon, and `vault` capability, failure, crash, idempotency, recovery, and surface matrices, including receipt audit records under `world/data/futureshops/receipts`, their matching backup and restore, unknown-record classification, and direct Pixelmon refusal before intent or custody where durable provider receipts and idempotent retry are absent.
...
11. A completed issue-guidance checklist proving issue 66 explains one economy gate, write-ahead identity and states, the durable receipt audit path `world/data/futureshops/receipts` and its local-evidence limit, clean marker, unknown-record `RECOVERING` or `FROZEN` handling, buy and sell custody ordering, durable claims, `READY`, `DRAINING`, `RECOVERING`, `FROZEN`, provider capability negotiation, direct Pixelmon no-mutation behavior without durable provider receipts and idempotent retry, the separate bridge contract, checksummed backup scope, evidence-based operator reconciliation, Forge `1.20.1` implementation work, and future `1.21.1` port revalidation.
