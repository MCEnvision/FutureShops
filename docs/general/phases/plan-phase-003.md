...
6. **Pixelmon native-account contract.** Pixelmon `9.4.0` balance, precheck, boolean `add`, and boolean `take` calls do not prove a crash-window result. The Forge `1.20.1` implementation must target the native `PlayerPartyStorage` account with a request-aware mixin, persist the request UUID, operation, amount, and outcome receipt beside `pixelDollars`, deduplicate retries, force a durable save, and refuse custom or hybrid accounts. A local journal or observed balance delta is not a substitute, and arbitrary direct Pixelmon or plugin calls remain outside the transaction contract.
7. **Bridge contract.** A separate bridge may register `vault` through the public provider API and owns Bukkit, Vault, hybrid runtime, and economy-plugin dependencies. Its bridge and backend must adapt exact integer values, provider lifecycle, durable request identity, receipt lookup, idempotent replay, and one-transaction persistence for the balance effect and provider receipt. The current PixelmonEconomyBridge and FinalEconomy path requires modification before mutation can be enabled. FutureShops core must not link, reflect into, bundle, or fall back through Bukkit or Vault.
...
| `DEC-018`, `EXT-008` | Pixelmon capability, native account, receipt, durable-save, and class-isolation checks; owner-acceptance chronology audit | Exact Pixelmon query, precheck, native mutation, and custom or hybrid refusal integration | Owner-authorized exact disposable Pixelmon runtime | Missing receipt, failed save, duplicate, missing idempotent retry, ambiguous result, unrecognized account, preacceptance launch refusal | Pixelmon capability report, native receipt and save matrix, safe-refusal matrix, environment manifest |
