...
Every administrator field and icon control has contextual help by hover or keyboard focus. Help mode keeps descriptions visible. Invalid controls receive an outline and short message. Section badges count unresolved issues, and selecting a validation summary entry returns focus to the affected control. Apply and Save and Close remain visible while blocking errors disable submission. Disabled help names the exact first issue and counts only additional issues. Server paths are reduced to the edited listing, so hidden listing, option, and component identifiers are not mislabeled as Display Name errors.

Apply waits for the matching server acknowledgement and keeps the editor open with the acknowledged snapshot as its new baseline. Save and Close returns only after that acknowledgement. Editing any rejected draft clears the stale response without requiring the editor to close. Every simple trade mode remains selectable for every item, and the current mode is shown as selected rather than disabled. Revert restores the acknowledged baseline. Duplicate creates a new stable listing identifier. Remove Option and Remove Listing use separate confirmations. A stale revision opens review choices and never overwrites the newer server listing silently.
