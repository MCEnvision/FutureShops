...
> **Owner:** Security and command owners
...
| SRC-013, DEC-006 | Repository rules require the full mandatory audits, non-destructive recovery, security-safe evidence, verification order, and merge discipline | All tasks | Completion packet proves every gate and records any unavailable mandatory evidence as a blocker |
...
**Dependencies:** CORE-PHASE-003, CORE-REQ-009, CORE-REQ-016, DEC-002, DEC-006, EXT-005
...
- `CORE-PHASE-003` has passed its integration gate, its Forge pull request is merged, and `origin/1.20.1` contains the merge commit.
...
- A read-only GitHub preflight confirms `EXT-005`, authoritative remote identity, issue and advisory visibility, review and check state, open security and dependency alerts, and no unresolved earlier-phase change that invalidates the integrated entry point.
...
- Sanitized fixtures, isolated server worlds, non-privileged and privileged test identities, permission-plugin and operator-level configurations, malformed payload generators, safe path fixtures, and artifact inspection tools are available without using real private player data.
...
13. `P004-TASK-013` runs focused and full deterministic verification, dedicated-server command exercises, client and multiplayer checks where applicable, and exact JAR inspection at each repaired line revision.
14. `P004-TASK-014` updates tracked documentation, sanitized issue or advisory evidence, and operator guidance to match merged behavior.
15. `P004-TASK-015` reruns the complete security, privacy, dependency, packet, command, path, codec, permission, duplication, and JAR matrices at the exact merged revisions and assembles the completion packet.
...
- Test, build, runtime, multiplayer where applicable, JAR listing, metadata, dependency, secret-scan, and full-diff results at each exact merged revision.
- Documentation and operator guidance tied to verified behavior without exploit-enabling detail or private data.

**Exit criteria**

- Every entry point, executable command leaf, permission path, packet, path, decoder, dependency, packaged component, and privacy-bearing output is represented in a frozen matrix for the exact merged Forge revision and, where present, the exact NeoForge revision.
- Every suspected finding is classified as disproven, accepted design with evidence, duplicate, verified defect, confidential blocker, or excluded scope. There is no unclassified concern.
...
- Every verified in-scope finding is repaired on every independently proven affected support line, merged through the correct pull request, and connected to failing and passing evidence.
...
- The threat, packet, command, path, serialization, permission, duplication, privacy, dependency, and JAR matrices rerun clean at the exact merged revisions.
...
| Approved NeoForge revision | `CORE-PHASE-001` and branch governance | Issue 22 internal integration state and current `1.21.1` head are explicit | Git ancestry, open blocker state, metadata, and prior evidence | Record blocker. Audit only a named approved revision and do not import Forge changes |
...
| P004-TASK-013 | CORE-REQ-010, CORE-REQ-011 | Verify each repaired line through full required local and runtime gates | P004-TASK-012 | Exact-revision verification results | Tests, data, GameTests where applicable, builds, server, client, multiplayer, JAR | Repository-prescribed order with decisive results retained |
| P004-TASK-014 | CORE-REQ-010, CORE-REQ-011, CORE-REQ-017 | Merge repairs and reconcile documentation and tracking safely | P004-TASK-013, EXT-005 | Merged revisions, safe records, updated docs and runbooks | Pull requests, issues or advisories, README, technical and focused docs | Green checks, private review where available, source-to-doc and disclosure review |
| P004-TASK-015 | CORE-REQ-010, CORE-REQ-011 | Rerun every Phase 004 matrix at exact merged revisions and prepare Phase 005 handoff | P004-TASK-014 | Clean final audit packet and persistence security inventory | Both supported lines and all Phase 004 matrices | Zero unclassified or open in-scope findings; revision binding verified |
...
- Repairs may proceed independently on separate support lines only after separate affected-line proof. Each line gets its own branch, regression, pull request, review, checks, merge, and post-merge rerun.
...
7. If private vulnerability reporting is unavailable, stop sensitive repair coordination before public disclosure. Record only a non-public blocker in the completion packet and restore the private channel; do not substitute a public exploit report.
...
| Sensitive finding cannot be recorded privately | Private advisory capability or authorization unavailable | Do not publish exploit details and do not begin public repair coordination | Record confidential blocker and restore approved private channel | Capability check before repair timestamp |
...
| P004-TASK-009 | Replay, fingerprint, checked arithmetic, concurrency property tests | Command and packet mutations through escrow and services | Retry, disconnect, restart, two-client concurrency where applicable | Duplicate, stale state, provider and delivery failure | Conservation and idempotency report |
...
| P004-TASK-015 | Complete matrix rerun | Cross-matrix consistency and issue-to-fix traceability | Exact merged runtime rerun | Any new finding reopens Task 011 and invalidates affected evidence | Clean Phase 004 completion packet |
...
- Use isolated Forge 1.20.1 Java 17 and NeoForge 1.21.1 Java 21 environments at named revisions. Never reuse a mutated world as the pristine baseline. Back up the complete world and FutureShops state as one matching generation before fault tests.
...
| Required private review or GitHub capability is unavailable | Check capability at entry and record optional versus mandatory gates | Authentication or feature failure | Continue deterministic work only where permitted; block sensitive routing if no private channel | Finding and merge evidence | Restore capability and rerun affected workflow |
...
1. The exact entry and merged Forge and NeoForge revisions, branch ancestry, toolchains, merge commits, and required green checks.
...
6. The finding register with classifications, duplicate searches, canonical issue or advisory timestamps, disclosure levels, affected lines, repairs, regressions, pull requests, reviews, merges, and closure or blocker states.
...
8. Full unit, applicable data and GameTest, build, dedicated-server, client, multiplayer where applicable, restart, reconnect, dependency, JAR, secret, and diff results at exact merged revisions.
...
13. A signed statement that no public exploit detail or private player evidence was exposed before safe remediation, no publication action occurred, and no known Phase 004 finding remains.

## Next Transition

After `CORE-PHASE-004` has merged every repair, rerun Task 015 on the actual support-line merge revisions, and assembled the complete packet, begin `CORE-PHASE-005` by reading `phases/plan-phase-005.md` through EOF. Freeze the complete persistence inventory against the exact merged Phase 004 Forge revision and the reviewed NeoForge revision where applicable. Carry forward every path, codec, serialization, privacy, authorization, duplication, journal, SavedData, configuration, catalog, NBT, JSON, TOML, backup, and recovery boundary discovered here.

Do not start `CORE-PHASE-005` from an open Phase 004 pull request, a pre-merge test commit, or a branch that does not descend from the verified support-line merge. A later Phase 005 change that affects a Phase 004 boundary invalidates the named Phase 004 matrix rows and requires their exact-revision rerun. Phase 005 owns persistence, migration, integrity, recovery, and economic conservation closure; it does not reopen a clean Phase 004 finding unless new evidence proves a defect, in which case the rolling `CORE-REQ-009` issue-before-repair loop applies again.
